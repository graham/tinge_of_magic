...
function loadDirectory(config_dict, dirs, prefix) {
    let cd = config_dict;

    for(let i = 0; i < dirs.length; i++) {
        cd = _loadDirectory(cd, dirs[i], prefix);
    }

    return cd;
}

function _loadDirectory(config_dict, dir, prefix) {
    var files;
    ...
    try {
        ...
        console.log(
            "webpack_config_test doesnt have access to the file data so we need a default", e
        );
        files = [];
    }
    ...
    files.forEach(function (file) {
        if (
            ...
            if (config_dict.entry[entry_name] !== undefined) {
                console.log(SKIP, 'SKIP ', REGULAR, 'Skipping ' + file + ' because of custom entry.');
                return;
            }
            ...
            if (fs.existsSync(main_path) == false) {
                ...
                console.log(PASS, "FOUND", REGULAR, "Adding " + entry_name);
