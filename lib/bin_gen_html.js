...
let template_payload = fs.readFileSync(template_path).toString();
...
let external_path_to_root = '';
if (argv['external_path_to_root'] != undefined) {
    external_path_to_root = argv['external_path_to_root'];
}

console.log('');

constants.pagesDirectories.forEach( (pd) => {
    let pageDir = pd;
    ...
    let config = util.loadDirectory({entry: []}, [pageDir], "");
